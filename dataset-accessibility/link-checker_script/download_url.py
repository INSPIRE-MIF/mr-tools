...
request_timeout = 10  # Adjust the timeout as needed
...
def check_url(url):
    try:
        ...
        if response.status_code == 200:
            ...
            # If HEAD method is not allowed, fall back to a GET request
...
ms = download.ms  # Replace with the appropriate value or variable for ms
input_csv = 'download/Download_' + ms + '.csv'
output_csv = 'download/Download_' + ms + '_url_check_results.csv'
failed_log = 'download/Download_' + ms + '_failed_requests.log'  # Log file for failed requests
